...
def mle_trev_given_pi(
  ...
  assert sktime.markov.tools.estimation.is_connected(C,directed=False), 'C must be (weakly) connected'
  ...
  if err == -1:
    ...
    warnings.warn('Reversible transition matrix estimation with fixed stationary distribution didn\'t converge.', sktime.markov.tools.util.exceptions.NotConvergedWarning)
