...
def mle_trev(C, double maxerr=1.0E-12, int maxiter=int(1.0E6),
  ...
  assert sktime.markov.tools.estimation.is_connected(C, directed=True), 'C must be strongly connected'
  ...
  T = sktime.markov.tools.estimation.sparse.transition_matrix.correct_transition_matrix(T)
  if return_statdist:
      return T, mu
  else:
      return T
