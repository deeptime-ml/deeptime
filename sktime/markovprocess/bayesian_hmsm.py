...
class BayesianHMMPosterior(Model):
    ...
    def submodel(self, states=None, obs=None, mincount_connectivity='1/n', inplace=False):
        # TODO: call submodel on MaximumLikelihoodHMSM?!?
...
class BayesianHMSM(_MaximumLikelihoodHMSM):
    ...
    def fit(self, dtrajs):
        ...
        if self.init_hmsm is None:  # estimate using maximum-likelihood superclass
            ...
            super(BayesianHMSM, self).fit(dtrajs)
            ...
            if self.stride == 'effective':
                self.stride = _HMMCountEstimator._compute_effective_stride(dtrajs, self.stride, self.lagtime, self.nstates)
        ...
        if self.nstates_obs < nstates_full:
            ...
            # fill active states
            B_init[:, self.observable_set] = np.maximum(eps, self.observation_probabilities)
        ...
        for P, pi, pobs in sample_inp:  # restrict to observable set if necessary
            ...
            samples.append(HMSM(P, pobs, pi=pi, dt_model=self.dt_model))

        # store results
        if self.store_hidden:
            self._model.sampled_trajs = [s.hidden_state_trajectories for s in sampled_hmm]
