...
estimators = [
    ('k-Means with uniform initialization', deeptime.clustering.KmeansClustering(
        n_clusters=100,  # place 100 cluster centers
        init_strategy='uniform',  # uniform initialization strategy
        fixed_seed=13,
        n_jobs=8)
     ),
    ('k-Means with k-means++ initialization', deeptime.clustering.KmeansClustering(
        n_clusters=100,  # place 100 cluster centers
        init_strategy='kmeans++',  # uniform initialization strategy
        fixed_seed=13,
        n_jobs=8)
     ),
    ('Regular space clustering', deeptime.clustering.RegularSpaceClustering(
