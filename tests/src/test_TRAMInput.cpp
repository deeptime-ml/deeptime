...
TEMPLATE_TEST_CASE("TRAMInput", "[tram][template]", std::vector<np_array_nfc<double>>) {
    GIVEN("Input") {
	...
	WHEN("TRAMInput is constructed") {
            ...
            auto input = deeptime::tram::TRAMInput<double>(std::move(stateCounts), std::move(transitionCounts), dtrajs, biasMatrices);
        ...
        WHEN("statecounts don't match transition counts at first index") {
	        ...
	        REQUIRE_THROWS_AS(deeptime::tram::TRAMInput<double>(std::move(stateCounts), std::move(transitionCounts), dtrajs, biasMatrices)
				, std::runtime_error);
	    }
	}
