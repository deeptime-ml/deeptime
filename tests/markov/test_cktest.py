...
def test_cktest_double_well(estimator_type):
    ...
    if estimator_type == "MLMSM":
        ...
        validator = est.chapman_kolmogorov_validator(2, mlags=[1, 10])
        ...
        bmsm_est.fit(counts)
        validator = bmsm_est.chapman_kolmogorov_validator(2, mlags=[1, 10])
        ...
        validator = hmm_est.chapman_kolmogorov_validator([1, 10], hmm_test.submodel_largest(dtrajs=dtraj))
        ...
        validator = bhmm_est.chapman_kolmogorov_validator([1, 10], bhmm)
    else:
        pytest.fail()
