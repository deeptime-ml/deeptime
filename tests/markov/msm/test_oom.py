...
def test_score_cv(five_state_msm, reversible, sparse):
    ...
    s1 = score_cv(dtrajs=five_state_msm.dtrajs[:500], lagtime=5, n=2, score_method='VAMP1', score_k=2, blocksplit=False,
                  fit_fetch=lambda dtrajs: msm.fit(dtrajs).fetch_model()).mean()
    np.testing.assert_(1.0 <= s1 <= 2.0)
    s2 = score_cv(dtrajs=five_state_msm.dtrajs[:500], lagtime=5, n=2, score_method='VAMP2', score_k=2, blocksplit=False,
                  fit_fetch=lambda dtrajs: msm.fit(dtrajs).fetch_model()).mean()
