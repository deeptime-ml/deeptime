...
class TestEstimators(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.pi = sktime.markov.tools.analysis.statdist(cls.T)
    ...
    def test_Kalbfleisch_Lawless_with_connectivity(self):
        est = sktime.markov.tools.estimation.dense.ratematrix.KalbfleischLawlessEstimator(self.C, self.K, self.pi, dt=self.tau, sparsity=self.C0, t_agg=self.t_agg*self.tau, tol=100.0)
        K_est = est.run()
        assert np.allclose(self.K, K_est, rtol=5.0E-3)

    def test_Crommelin_Vanden_Eijnden_with_connectivity(self):
        est = sktime.markov.tools.estimation.dense.ratematrix.CrommelinVandenEijndenEstimator(self.T, self.K, self.pi, dt=self.tau, sparsity=self.C0, t_agg=self.t_agg*self.tau, tol=100.0)
        K_est = est.run()
        assert np.allclose(self.K, K_est, rtol=5.0E-3)

    def test_api_with_connectivity_with_pi(self):
        K_est = sktime.markov.tools.estimation.rate_matrix(self.C, dt=self.tau, sparsity=self.C0, t_agg=self.t_agg*self.tau, pi=self.pi, tol=100.0)
        assert np.allclose(self.K, K_est, rtol=5.0E-3)
    ...
    def test_api_with_connectivity_without_pi(self):
        K_est = sktime.markov.tools.estimation.rate_matrix(self.C, dt=self.tau, sparsity=self.C0, t_agg=self.t_agg*self.tau, tol=100.0)
        assert np.allclose(self.K, K_est, rtol=5.0E-3)
    ...
    def test_api_with_connectivity_with_pi_with_guess(self):
        K_est = sktime.markov.tools.estimation.rate_matrix(self.C, dt=self.tau, sparsity=self.C0, t_agg=self.t_agg*self.tau, pi=self.pi, tol=100.0, K0=self.K)
        assert np.allclose(self.K, K_est, rtol=5.0E-3)
    ...
    def test_api_with_connectivity_without_pi_with_guess(self):
        K_est = sktime.markov.tools.estimation.rate_matrix(self.C, dt=self.tau, sparsity=self.C0, t_agg=self.t_agg*self.tau, tol=100.0, K0=self.K)
        assert np.allclose(self.K, K_est, rtol=5.0E-3)
    ...
    def test_warn(self):
        with warnings.catch_warnings(record=True) as w:
            ...
            warnings.simplefilter('always', category=sktime.markov.tools.estimation.dense.ratematrix.NotConvergedWarning)
